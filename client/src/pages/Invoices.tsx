...
import { getInvoices, downloadPdf } from '../services/api';
              ...
              <table className="w-full min-w-[800px]">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">N° Facture</th>
                    ...
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Paiement</th>
                    <th className="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Total</th>
                    <th className="text-right px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Payé</th>
                    <th className="text-center px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Statut</th>
                  ...
                  {invoices.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="text-center py-12 text-gray-400">
                        Aucune facture.{' '}
                        <Link to="/factures/nouvelle" className="text-blue-600 underline">
                          Créer une facture
                        </Link>
                      </td>
                    </tr>
                    ...
                    invoices.map((inv) => (
                      <tr key={inv.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 font-medium text-gray-800">{inv.number}</td>
                        <td className="px-6 py-4 text-gray-600">{inv.clientName}</td>
                        <td className="px-6 py-4 text-gray-600">{new Date(inv.date).toLocaleDateString('fr-FR')}</td>
                        <td className="px-6 py-4 text-gray-600 text-sm">{paymentLabels[inv.paymentMethod] || inv.paymentMethod}</td>
                        <td className="px-6 py-4 text-right font-semibold text-gray-800">{formatCFA(inv.total)}</td>
                        <td className="px-6 py-4 text-right text-gray-600">{formatCFA(inv.amountPaid)}</td>
                        <td className="px-6 py-4 text-center">
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusColors[inv.status]}`}>
                        ...
                        <td className="px-6 py-4 text-right space-x-1">
                          <button
                            ...
                            className="inline-block px-3 py-1 bg-blue-100 text-blue-700 rounded text-xs font-semibold hover:bg-blue-200 transition-colors"
                          >
                            PDF
                          </button>
                          {inv.clientPhone && (
              ...
              invoices.map((inv) => (
                <div key={inv.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
                  ...
                  <div className="flex items-center justify-between text-sm mb-3">
                    <span className="text-gray-500">{new Date(inv.date).toLocaleDateString('fr-FR')} · {paymentLabels[inv.paymentMethod]}</span>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => downloadPdf(inv.id, `${inv.number}.pdf`).catch(() => toast.error('Erreur PDF'))}
                      className="flex-1 py-2 bg-blue-100 text-blue-700 rounded-lg text-xs font-semibold hover:bg-blue-200 transition-colors text-center"
                    >
                      📄 PDF
                    </button>
                    {inv.clientPhone && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex-1 py-2 bg-green-100 text-green-700 rounded-lg text-xs font-semibold hover:bg-green-200 transition-colors text-center"
                      >
                        💬 WhatsApp
                      </a>
                    )}
