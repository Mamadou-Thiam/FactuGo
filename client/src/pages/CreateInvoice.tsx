...
const emptyItem: InvoiceItem = { designation: '', quantity: 1, unitPrice: 0, discount: 0, imeiVendeur: '', imeiAcheteur: '' };
...
export default function CreateInvoice() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (items.some((item) => !item.imeiVendeur || item.imeiVendeur.trim() === '')) {
      toast.error('IMEI du vendeur requis pour chaque article');
      return;
    }
  ...
  if (createdInvoice) {
    ...
    const handleWhatsApp = async () => {
      try {
        const blob = await getInvoicePdfBlob(createdInvoice.id);
        const file = new File([blob], `${createdInvoice.number}.pdf`, { type: 'application/pdf' });
      ...
      window.open(`https://wa.me/221${cleanPhone}?text=${encodeURIComponent(whatsappText)}`, '_blank');
            ...
            {items.map((item, index) => (
              <div key={index} className="bg-gray-50 rounded-lg p-3 sm:p-4 space-y-2 sm:space-y-0 sm:grid sm:grid-cols-12 sm:gap-3 sm:items-end">
                ...
                <div className="sm:col-span-5 space-y-1">
                  {index === 0 && <label className="block text-xs text-gray-500">Désignation</label>}
                  <select
                ...
                <div className="sm:col-span-12 grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <div>
                    {index === 0 && <label className="block text-xs text-gray-500 mb-1">IMEI Vendeur *</label>}
                    <input
                      type="text"
                  ...
                  <div>
                    {index === 0 && <label className="block text-xs text-gray-500 mb-1">IMEI Acheteur</label>}
                    <input
                      type="text"
